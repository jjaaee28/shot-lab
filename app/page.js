...
export default function Home(){
  ...
  return <main>
    ...
    <section className={loginPage?'login-page':entered?'workspace':scriptPage?'script-page':'intro'}>
      <div className="heading"><div><p className="eyebrow">{loginPage?'WELCOME TO SHOT LAB':entered?'THE CAMERA IS YOURS':scriptPage?'SCRIPT TO SCENE / DEMO':'A SMALL STUDY OF PERSPECTIVE'}</p><h1>{loginPage?'촬영 구도 실험실':entered?'카메라를 움직여 보세요.':scriptPage?'장면을 글로 써보세요.':<>같은 장면,<br/>다른 카메라.</>}</h1></div>{entered?<button className="text-button" onClick={openIntro}>소개로 돌아가기 ↗</button>:scriptPage?<button className="text-button" onClick={openIntro}>소개로 돌아가기 ↗</button>:loginPage?<span className="edition">DEMO ACCESS <span>/</span> 001</span>:<p className="intro-copy">촬영을 처음 배우는 사람을 위한 작은 실험실.<br/>각도와 움직임만 바꿔도, 장면은 달라집니다.</p>}</div>
