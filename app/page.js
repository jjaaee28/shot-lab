...
export default function Home(){
  ...
  function saveCurrentShot(){const s=state.current;const m=METHODS.find(item=>item.id===s.method);setSavedShots(prev=>[{id:`shot-${Date.now()}`,title:`${m?.name||'저장한 구도'} · ${prev.length+1}`,method:s.method,angle:s.angle,elevation:s.elevation,zoom:s.zoom,panX:s.panX,panY:s.panY,preset:s.preset},...prev]);}
